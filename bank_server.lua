...
local function validPin(pin)
    return tostring(pin or ""):match("^%d%d%d%d$") ~= nil
end
...
local function process(m)
    ...
    if m.action=="set_card_status" then
        ...
        if d.status~="active" and d.status~="blocked" then return response(id,false,nil,"INVALID_STATUS") end
        c.status=d.status
        if not save(cardPath(c.id),c) then return response(id,false,nil,"CARD_FILE_ERROR") end
        return response(id,true,{card_id=c.id,status=c.status})
    end
